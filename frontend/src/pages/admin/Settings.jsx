...
export default function Settings() {
  ...
  const testApiKey = async () => {
    if (!settings.tmdb_api_key) return setTestResult({ ok: false, msg: 'Introduce la API Key primero' });
    setTesting(true);
    setTestResult(null);
    try {
      await api.put('/admin/settings', { tmdb_api_key: settings.tmdb_api_key });
      const res = await api.get('/movies/popular');
      setTestResult({ ok: true, msg: `✓ API Key válida. Se encontraron ${res.data.results?.length || 0} películas.` });
    } catch (e) {
      setTestResult({ ok: false, msg: e.response?.data?.error || 'API Key inválida o error de conexión' });
    } finally {
      setTesting(false);
    }
  };
      ...
      <form onSubmit={handleSave} className="space-y-6">
              ...
              <div className="flex gap-3">
                {[['true', 'Habilitado ✓'], ['false', 'Deshabilitado ✗']].map(([val, label]) => (
                  <button
                    key={val}
                    type="button"
                    onClick={() => setSettings({ ...settings, allow_register: val })}
        ...
        <div className="glass-card p-6">
          ...
          <p className="text-gray-500 text-sm mb-5 pb-3 border-b border-gray-800">
            ...
            <a href="https://www.themoviedb.org/settings/api" target="_blank" rel="noopener noreferrer" className="text-accent-light hover:underline">
              themoviedb.org
            </a>
          </p>
          <div>
            ...
            <button
              ...
              onClick={testApiKey}
              disabled={testing}
              className="mt-3 px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 hover:text-white text-sm rounded-lg transition-colors flex items-center gap-2"
            >
