...
const getApiKey = () => db.settings.get('tmdb_api_key') || process.env.TMDB_API_KEY;
...
router.get('/movies/trending', async (req, res) => {
  try { res.json(await tmdb('/trending/movie/week', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/movies/popular', async (req, res) => {
  try { res.json(await tmdb('/movie/popular', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/movies/top-rated', async (req, res) => {
  try { res.json(await tmdb('/movie/top_rated', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/movies/now-playing', async (req, res) => {
  try { res.json(await tmdb('/movie/now_playing', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/movies/upcoming', async (req, res) => {
  try { res.json(await tmdb('/movie/upcoming', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/movies/genre/:id', async (req, res) => {
  try { res.json(await tmdb('/discover/movie', { with_genres: req.params.id, page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/series/trending', async (req, res) => {
  try { res.json(await tmdb('/trending/tv/week', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/series/popular', async (req, res) => {
  try { res.json(await tmdb('/tv/popular', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/series/top-rated', async (req, res) => {
  try { res.json(await tmdb('/tv/top_rated', { page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
router.get('/series/genre/:id', async (req, res) => {
  try { res.json(await tmdb('/discover/tv', { with_genres: req.params.id, page: req.query.page || 1 })); }
  catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/search', async (req, res) => {
  try {
    ...
    res.json(await tmdb(`/search/${type}`, { query: q, page }));
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
router.get('/genres/series', async (req, res) => {
  try { res.json(await tmdb('/genre/tv/list')); }
  catch (e) { res.status(500).json({ error: e.message }); }
});

// Watchlist
router.get('/watchlist', auth, (req, res) => {
  res.json(db.watchlist.findByUser(req.user.id));
});
router.post('/watchlist', auth, (req, res) => {
  const { tmdb_id, media_type, title, poster_path } = req.body;
  db.watchlist.add(req.user.id, { tmdb_id, media_type, title, poster_path });
  res.json({ message: 'Agregado a Mi Lista' });
});
router.delete('/watchlist/:tmdb_id/:media_type', auth, (req, res) => {
  db.watchlist.remove(req.user.id, req.params.tmdb_id, req.params.media_type);
